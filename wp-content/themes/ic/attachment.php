 ...
 get_header(); ?>
					...
					<article class="hs-content" id="pagecontent">
						...
						<?php if ( wp_attachment_is_image( $post->id ) ) : $att_image = wp_get_attachment_image_src( $post->id, "large"); ?>
      						<p class="attachment"><a href="<?php echo wp_get_attachment_url($post->id); ?>" title="<?php wp_title(); ?>" rel="attachment"><img src="<?php echo $att_image[0];?>" style="max-width: 100%;" class="attachment-medium" alt="<?php $post->post_excerpt; ?>" /></a>
     						 </p>
						<?php else : ?>  
