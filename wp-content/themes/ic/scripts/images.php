...
jQuery(function($){
				...
				$.supersized({
					...
					{image : '<?php $imgsrc = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), "Full"); if($imgsrc) { echo $imgsrc[0]; } else { ?><?php bloginfo ('template_url'); ?>/images/nopic.jpg<? } ?>', title : '<?php if(get_post_meta($post->ID, 'heading',true)) { ?><div class="major"><?php echo get_post_meta($post->ID, 'heading',true); ?></div><? } ?><?php if(get_post_meta($post->ID, 'desc',true)) { ?><div class="slidedescription"><?php echo get_post_meta($post->ID, 'desc',true); ?></div><?php } ?><?php if(get_post_meta($post->ID, 'button1name',true)) { ?><ul class="footprints"><?php if(get_post_meta($post->ID, 'button1name',true)) { ?><li><a href="<?php echo get_post_meta($post->ID, 'button1link',true); ?>"><?php echo get_post_meta($post->ID, 'button1name',true); ?></a></li><?php } ?><?php if(get_post_meta($post->ID, 'button2name',true)) { ?><li><a href="<?php echo get_post_meta($post->ID, 'button2link',true); ?>"><?php echo get_post_meta($post->ID, 'button2name',true); ?></a></li><? } ?><?php if(get_post_meta($post->ID, 'button3name',true)) { ?><li><a href="<?php echo get_post_meta($post->ID, 'button3link',true); ?>"><?php echo get_post_meta($post->ID, 'button3name',true); ?></a></li><? } ?></ul><? } ?>', thumb : '', url : '//cebocampbell.com'}<?php if(($query->current_post + 1) == ($query->post_count)) { ?> <? } else { ?>,<? } ?>
