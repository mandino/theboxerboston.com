...
class TraceMiddleware
{
    ...
    public function __invoke($step, $name)
    {
        ...
        return function (callable $next) use($step, $name) {
            return function (\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request = null) use($next, $step, $name) {
    ...
    private function commandArray(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $cmd)
    {
        return ['instance' => spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $cmd->toArray()];
    }
    private function requestArray(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function responseArray(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\ResponseInterface $response = null)
    {
    ...
    private function streamStr(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\StreamInterface $body)
    {
        return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
    }
    private function createHttpDebug(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command)
    {
        if ($this->config['http'] && !isset($command['@http']['debug'])) {
            $command['@http']['debug'] = fopen('php://temp', 'w+');
        }
    }
    private function flushHttpDebug(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command)
    {
