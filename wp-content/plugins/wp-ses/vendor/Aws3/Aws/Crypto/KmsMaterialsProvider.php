...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Kms\KmsClient;
/**
 * Uses KMS to supply materials for encrypting and decrypting data.
 */
class KmsMaterialsProvider extends \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Crypto\MaterialsProvider
{
    private $kmsClient;
    private $kmsKeyId;
    /**
     ...
     *                         and decrypting keys.
     */
    public function __construct(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Kms\KmsClient $kmsClient, $kmsKeyId = null)
    {
        $this->kmsClient = $kmsClient;
        $this->kmsKeyId = $kmsKeyId;
    }
    public function fromDecryptionEnvelope(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Crypto\MetadataEnvelope $envelope)
    {
        if (empty($envelope[\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER])) {
            throw new \RuntimeException('Not able to detect kms_cmk_id from an' . ' empty materials description.');
        }
        $materialsDescription = json_decode($envelope[\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER], true);
        ...
        return new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Crypto\KmsMaterialsProvider($this->kmsClient, $materialsDescription['kms_cmk_id']);
    }
    /**
