...
class RetryMiddleware
{
    ...
    public static function createDefaultDecider($maxRetries = 3)
    {
        ...
        return function ($retries, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\ResultInterface $result = null, $error = null) use($maxRetries, $retryCurlErrors) {
     ...
     */
    public function __invoke(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request = null)
    {
        $retries = 0;
        $requestStats = [];
        ...
        $g = function ($value) use($handler, $decider, $delay, $command, $request, &$retries, &$requestStats, &$g) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                if (!$decider($retries, $command, $request, null, $value)) {
                    return \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\rejection_for($this->bindStatsToReturn($value, $requestStats));
                }
