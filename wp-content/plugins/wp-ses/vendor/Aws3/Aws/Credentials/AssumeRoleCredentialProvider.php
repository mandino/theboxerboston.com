...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Sts\StsClient;
...
class AssumeRoleCredentialProvider
{
    ...
    {
        $client = $this->client;
        return $client->assumeRoleAsync($this->assumeRoleParams)->then(function (\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Result $result) {
            return $this->client->createCredentials($result);
        })->otherwise(function (\RuntimeException $exception) {
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CredentialsException("Error in retrieving assume role credentials.", 0, $exception);
        });
    }
}
