...
class InstanceProfileProvider
{
    ...
    {
        return \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\coroutine(function () {
            if (!$this->profile) {
                $this->profile = (yield $this->request(self::CRED_PATH));
            }
            $json = (yield $this->request(self::CRED_PATH . $this->profile));
            $result = $this->decodeResult($json);
            (yield new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\Credentials($result['AccessKeyId'], $result['SecretAccessKey'], $result['Token'], strtotime($result['Expiration'])));
        });
    }
    /**
     * @param string $url
    ...
    private function request($url)
    {
        ...
        if (strcasecmp($disabled, 'true') === 0) {
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CredentialsException($this->createErrorMessage('EC2 metadata server access disabled'));
        }
        $fn = $this->client;
        $request = new \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Psr7\Request('GET', self::SERVER_URI . $url);
        return $fn($request, ['timeout' => $this->timeout])->then(function (\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\ResponseInterface $response) {
            return (string) $response->getBody();
        })->otherwise(function (array $reason) {
            $reason = $reason['exception'];
            $msg = $reason->getMessage();
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CredentialsException($this->createErrorMessage($msg));
        });
    }
    ...
    private function decodeResult($response)
    {
        ...
        if ($result['Code'] !== 'Success') {
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CredentialsException('Unexpected instance profile ' . 'response code: ' . $result['Code']);
        }
        return $result;
    }
}
