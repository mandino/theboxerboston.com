...
class EcsCredentialProvider
{
    ...
    public function __invoke()
    {
        ...
        return $client($request, ['timeout' => $this->timeout])->then(function (\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\ResponseInterface $response) {
            ...
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CredentialsException("Error retrieving credential from ECS ({$msg})");
        });
    }
    /**
    ...
    private function decodeResult($response)
    {
        ...
        if (!isset($result['AccessKeyId'])) {
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CredentialsException('Unexpected ECS credential value');
        }
        return $result;
    }
}
