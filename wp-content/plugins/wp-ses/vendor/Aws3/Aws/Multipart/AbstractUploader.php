...
use DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\StreamInterface as Stream;
abstract class AbstractUploader extends \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Multipart\AbstractUploadManager
{
     ...
     * @param array  $config
     */
    public function __construct(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\AwsClientInterface $client, $source, array $config = [])
    {
     ...
     */
    protected function limitPartStream(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\StreamInterface $stream)
    {
        // Limit what is read from the stream to the part size.
        return new \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Psr7\LimitStream($stream, $this->state->getPartSize(), $this->source->tell());
    }
    ...
    private function determineSource($source)
    {
        ...
        if (is_string($source)) {
            $source = \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Psr7\try_fopen($source, 'r');
        }
        // Create a source stream.
        $stream = \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Psr7\stream_for($source);
