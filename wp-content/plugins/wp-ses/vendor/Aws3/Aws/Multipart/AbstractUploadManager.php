...
namespace DeliciousBrains\WP_Offload_SES\Aws3\Aws\Multipart;

use DeliciousBrains\WP_Offload_SES\Aws3\Aws\AwsClientInterface as Client;
...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\MultipartUploadException;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Result;
 ...
 * @internal
 */
abstract class AbstractUploadManager implements \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_CONCURRENCY = 5;
    /** @var array Default values for base multipart configuration */
    private static $defaultConfig = ['part_size' => null, 'state' => null, 'concurrency' => self::DEFAULT_CONCURRENCY, 'prepare_data_source' => null, 'before_initiate' => null, 'before_upload' => null, 'before_complete' => null, 'exception_class' => 'DeliciousBrains\\WP_Offload_SES\\Aws3\\Aws\\Exception\\MultipartUploadException'];
     ...
     * @param array  $config
     */
    public function __construct(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\AwsClientInterface $client, array $config = [])
    {
        $this->client = $client;
    ...
    public function promise()
    {
        ...
        return $this->promise = \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\coroutine(function () {
            // Initiate the upload.
            ...
            if (!$this->state->isInitiated()) {
                ...
                $this->state->setStatus(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Multipart\UploadState::INITIATED);
            }
            ...
            $commands = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandPool($this->client, $this->getUploadCommands($resultHandler), ['concurrency' => $this->config['concurrency'], 'before' => $this->config['before_upload']]);
            ...
            $this->state->setStatus(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Multipart\UploadState::COMPLETED);
     ...
     */
    protected abstract function handleResult(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\ResultInterface $result);
    /**
    ...
    private function determineState()
    {
        ...
        $state = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Multipart\UploadState($id);
    ...
    protected function getResultHandler(&$errors = [])
    {
        return function (callable $handler) use(&$errors) {
            return function (\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request = null) use($handler, &$errors) {
                return $handler($command, $request)->then(function (\DeliciousBrains\WP_Offload_SES\Aws3\Aws\ResultInterface $result) use($command) {
                    $this->handleResult($command, $result);
                    return $result;
                }, function (\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\AwsException $e) use(&$errors) {
                    $errors[$e->getCommand()[$this->info['part_num']]] = $e;
                    return new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Result();
                });
            };
        };
    }
