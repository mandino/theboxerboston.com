...
namespace DeliciousBrains\WP_Offload_SES\Aws3\Aws\Signature;

use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\CredentialsInterface;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CouldNotCreateChecksumException;
 ...
 */
class SignatureV4 implements \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
    ...
    public function signRequest(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    public function presign(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\CredentialsInterface $credentials, $expires, array $options = [])
    {
     ...
     */
    public static function convertPostToGet(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Psr7\stream_for(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Psr7\hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    protected function getPresignedPayload(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function createPresignedRequest(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Psr7\parse_query($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Psr7\build_query($req['query']));
        }
        return new \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
}
