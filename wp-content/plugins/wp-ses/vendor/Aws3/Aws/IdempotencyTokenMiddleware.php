...
class IdempotencyTokenMiddleware
{
     ...
     * @return callable
     */
    public static function wrap(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Service $service, callable $bytesGenerator = null)
    {
        return function (callable $handler) use($service, $bytesGenerator) {
            return new self($handler, $service, $bytesGenerator);
        };
    }
    public function __construct(callable $nextHandler, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Service $service, callable $bytesGenerator = null)
    {
