...
class Validator
{
     ...
     */
    public function validate($name, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Shape $shape, array $input)
    {
    ...
    private function dispatch(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Shape $shape, $value)
    {
    ...
    private function check_structure(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\StructureShape $shape, $value)
    {
    ...
    private function check_list(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\ListShape $shape, $value)
    {
        if (!is_array($value)) {
            $this->addError('must be an array. Found ' . \DeliciousBrains\WP_Offload_SES\Aws3\Aws\describe_type($value));
            return;
        }
    ...
    private function check_map(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\MapShape $shape, $value)
    {
        if (!$this->checkAssociativeArray($value)) {
            return;
        }
    ...
    private function check_blob(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Shape $shape, $value)
    {
        static $valid = ['string' => true, 'integer' => true, 'double' => true, 'resource' => true];
        $type = gettype($value);
        if (!isset($valid[$type])) {
            if ($type != 'object' || !method_exists($value, '__toString')) {
                $this->addError('must be an fopen resource, a ' . 'DeliciousBrains\\WP_Offload_SES\\Aws3\\GuzzleHttp\\Stream\\StreamInterface object, or something ' . 'that can be cast to a string. Found ' . \DeliciousBrains\WP_Offload_SES\Aws3\Aws\describe_type($value));
            }
        }
    }
    private function check_numeric(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Shape $shape, $value)
    {
        if (!is_numeric($value)) {
            $this->addError('must be numeric. Found ' . \DeliciousBrains\WP_Offload_SES\Aws3\Aws\describe_type($value));
            return;
        }
        $this->validateRange($shape, $value, "numeric value");
    }
    private function check_boolean(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Shape $shape, $value)
    {
        if (!is_bool($value)) {
            $this->addError('must be a boolean. Found ' . \DeliciousBrains\WP_Offload_SES\Aws3\Aws\describe_type($value));
        }
    }
    private function check_string(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Shape $shape, $value)
    {
        if ($shape['jsonvalue']) {
            if (!self::canJsonEncode($value)) {
                $this->addError('must be a value encodable with \'json_encode\'.' . ' Found ' . \DeliciousBrains\WP_Offload_SES\Aws3\Aws\describe_type($value));
            }
            return;
        }
        if (!$this->checkCanString($value)) {
            $this->addError('must be a string or an object that implements ' . '__toString(). Found ' . \DeliciousBrains\WP_Offload_SES\Aws3\Aws\describe_type($value));
            return;
        }
    ...
    private function validateRange(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Shape $shape, $length, $descriptor)
    {
    ...
    private function checkAssociativeArray($value)
    {
        if (!is_array($value) || isset($value[0])) {
            $this->addError('must be an associative array. Found ' . \DeliciousBrains\WP_Offload_SES\Aws3\Aws\describe_type($value));
            return false;
        }
        return true;
    }
