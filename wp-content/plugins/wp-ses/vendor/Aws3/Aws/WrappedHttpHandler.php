...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Parser\Exception\ParserException;
...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'DeliciousBrains\\WP_Offload_SES\\Aws3\\Aws\\Exception\\AwsException', $collectStats = false)
    {
     ...
     */
    public function __invoke(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $fn = $this->httpHandler;
        ...
        return \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command, array $stats)
    {
