 ...
 */
class EachPromise implements \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = false;
        $this->aggregate = new \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\Promise(function () {
            reset($this->pending);
            ...
            while ($promise = current($this->pending)) {
                next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
