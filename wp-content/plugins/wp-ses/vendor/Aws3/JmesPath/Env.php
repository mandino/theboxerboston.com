...
final class Env
{
    ...
    public static function search($expression, $data)
    {
        static $runtime;
        if (!$runtime) {
            $runtime = \DeliciousBrains\WP_Offload_SES\Aws3\JmesPath\Env::createRuntime();
        }
    ...
    public static function createRuntime()
    {
        switch ($compileDir = getenv(self::COMPILE_DIR)) {
            case false:
                return new \DeliciousBrains\WP_Offload_SES\Aws3\JmesPath\AstRuntime();
            case 'on':
                return new \DeliciousBrains\WP_Offload_SES\Aws3\JmesPath\CompilerRuntime();
            default:
                return new \DeliciousBrains\WP_Offload_SES\Aws3\JmesPath\CompilerRuntime($compileDir);
        }
    }
    /**
