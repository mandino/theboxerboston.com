Hustle.define("Settings.E_News", function($){
	...
	return Backbone.View.extend({
		...
		events: {
			...
			"click .optin-enews-double-optin-toggle" : "dtoggle"
		},
		initialize: function(){
			...
			$.ajax({
				...
				success: function( res ){
					if( res.success ){
						...
						if( state === false ){
							$("#double-optin-enews-state-" + id).prop({ checked: false, disabled: true }); 
						} else{
							$("#double-optin-enews-state-" + id).prop('disabled', false);
						}
					}
		...
		dtoggle: function(e){
			...
			$.ajax({
				...
				type: "POST",
				data: {
					action: "inc_opt_toggle_enews_double_optin",
					_ajax_nonce: nonce,
					id: id,
					state: state
				},
