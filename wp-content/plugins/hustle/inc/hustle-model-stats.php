...
class Hustle_Model_Stats extends Hustle_Data {
	...
	public function get_views_count(){
		return (int) $this->_wpdb->get_var( $this->_wpdb->prepare( "SELECT COUNT(meta_id) FROM " . Hustle_Db::modules_meta_table() . " WHERE `module_id`=%d AND `meta_key`=%s ", $this->_module->id,  $this->_get_key( self::KEY_VIEW ) ) );
	}
	...
	public function get_conversions_count(){
		return (int) $this->_wpdb->get_var( $this->_wpdb->prepare( "SELECT COUNT(meta_id) FROM " . Hustle_Db::modules_meta_table() . " WHERE `module_id`=%d AND `meta_key`=%s ", $this->_module->id,  $this->_get_key( self::KEY_CONVERSION )  ) );
	}
	...
	public function get_conversion_data(){
		return (object) $this->_wpdb->get_results( $this->_wpdb->prepare( "SELECT * FROM " . Hustle_Db::modules_meta_table() . " WHERE `module_id`=%d AND `meta_key`=%s ", $this->_module->id,  $this->_get_key( self::KEY_CONVERSION )  ) );
	}
}
